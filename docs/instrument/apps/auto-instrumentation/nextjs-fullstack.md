...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"How do I instrument both the browser and the server in one Next.js app?","acceptedAnswer":{"@type":"Answer","text":"Use two SDKs with two service names. The server side loads the Node SDK from the register() hook in instrumentation.ts, which Next.js calls once per runtime before any request is handled. The browser side loads the web SDK from a client component mounted in the root layout, so it starts on hydration. They emit under different service names and link through the W3C traceparent header on fetch calls."}},{"@type":"Question","name":"Why does instrumentation.ts check NEXT_RUNTIME?","acceptedAnswer":{"@type":"Answer","text":"Next.js calls register() once for every server runtime, including Edge. The Node SDK depends on Node.js built-ins that do not exist in the Edge runtime, so importing it there throws at startup. Guarding with process.env.NEXT_RUNTIME === 'nodejs' and using a dynamic import keeps the Node SDK out of the Edge bundle."}},{"@type":"Question","name":"Can I avoid configuring CORS on the collector for Next.js browser telemetry?","acceptedAnswer":{"@type":"Answer","text":"Yes. Add a catch-all API route at /api/otel/[...signal] that forwards the OTLP request body to the collector, and point the browser exporter at /api/otel instead of the collector origin. The browser then posts same-origin, so there is no preflight and no cors block on the OTLP HTTP receiver. The collector never needs to be reachable from the public internet."}},{"@type":"Question","name":"Why are my Next.js SSR errors missing from OpenTelemetry logs?","acceptedAnswer":{"@type":"Answer","text":"Next.js catches uncaught server-side exceptions itself and prints them to stdout rather than rethrowing them into a span. Nothing in the OTel SDK observes stdout, so the error never reaches the logs pipeline. Wrapping console.log, console.warn, and console.error after sdk.start() and re-emitting each call through the OTel logger captures that output as log records."}},{"@type":"Question","name":"How much overhead does OpenTelemetry add to a Next.js app?","acceptedAnswer":{"@type":"Answer","text":"Server-side, expect roughly 0.5-2ms added latency per request, 2-5% CPU during export, and 15-40MB of resident memory for the SDK and its span buffers. Browser-side, the web SDK adds roughly 90-130KB gzipped to the client bundle. All signals export off the request path through batch processors and a periodic metric reader, so export cost does not appear in user-facing latency."}},{"@type":"Question","name":"Why do I see two services for one Next.js app?","acceptedAnswer":{"@type":"Answer","text":"The server SDK and the browser SDK report under separate service names. They have different lifecycles, resource attributes, and failure modes. Separate names let you filter browser telemetry out of backend dashboards, and the two still join through a shared trace id."}},{"@type":"Question","name":"How do I capture React error boundary crashes with OpenTelemetry?","acceptedAnswer":{"@type":"Answer","text":"A window.onerror listener does not see them, because React catches render errors before they reach the window. Export a reporting helper from your browser telemetry module and call it from a useEffect inside the App Router error.tsx and global-error.tsx boundaries. The helper opens a span, sets an ERROR status, and attaches the error message, stack, and component stack."}},{"@type":"Question","name":"Should I record Core Web Vitals as spans or metrics in Next.js?","acceptedAnswer":{"@type":"Answer","text":"Metric histograms aggregate better, because Core Web Vitals are scored as fleet-wide distributions at p75 and a histogram lets the backend compute percentiles without storing every event. Recording them as spans, as this example does, is simpler to wire up and keeps each vital attached to the session that produced it, which is useful when debugging one slow page rather than tracking a fleet trend."}},{"@type":"Question","name":"Why is my Next.js browser span in a different trace than my API route span?","acceptedAnswer":{"@type":"Answer","text":"The traceparent header is not reaching the server. Check that the request URL matches propagateTraceHeaderCorsUrls in the fetch instrumentation config. If the browser posts to a different origin than the page, the server's CORS policy must also allow the traceparent and tracestate request headers, otherwise the browser strips them before the request leaves."}},{"@type":"Question","name":"Do I need output standalone for OpenTelemetry to work in a Next.js Docker image?","acceptedAnswer":{"@type":"Answer","text":"Standalone output is not required, but if you use it you must copy the full .next/server directory into the runtime image alongside the standalone bundle. The instrumentation hook is compiled into a separate server chunk that the standalone trace does not always include, so omitting it produces an image that starts cleanly and emits nothing."}},{"@type":"Question","name":"Does OpenTelemetry work with the Next.js Edge runtime?","acceptedAnswer":{"@type":"Answer","text":"Not with the Node SDK. The Edge runtime is a restricted JavaScript environment without the Node.js built-ins the SDK requires. Guard the register hook so the Node SDK loads only under the nodejs runtime, and instrument Edge routes with the OpenTelemetry API alone or move them to the Node runtime if you need full auto-instrumentation."}},{"@type":"Question","name":"Which Next.js version does this guide target?","acceptedAnswer":{"@type":"Answer","text":"The example runs Next.js 16.3 with React 19.2 on Node.js 22. The instrumentation hook has been stable since Next.js 15, so the server-side setup applies unchanged from 15 onward. The browser-side setup has no Next.js version dependency beyond the App Router and client components."}},{"@type":"Question","name":"How is this different from the Next.js server-side guide?","acceptedAnswer":{"@type":"Answer","text":"The Next.js server-side guide covers a server-only application with MongoDB and BullMQ background jobs, and is the right starting point if your API tier is the concern. This guide covers the browser and server together in one app, with the OTLP proxy route, browser error capture, and Web Vitals that a server-only setup does not need."}}]})}
  </script>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"HowTo","name":"Instrument a Next.js App Router application with OpenTelemetry across the browser and the server","step":[{"@type":"HowToStep","name":"Install packages","text":"Install the Node SDK and node auto-instrumentations for the server, the web SDK and web auto-instrumentations for the browser, the three OTLP HTTP exporters, and web-vitals."},{"@type":"HowToStep","name":"Add the register hook","text":"Create instrumentation.ts at the project root and dynamically import the server telemetry module only when NEXT_RUNTIME is nodejs, so the Node SDK never loads in the Edge runtime."},{"@type":"HowToStep","name":"Initialize server telemetry","text":"Configure the NodeSDK with a resource, a BatchSpanProcessor, a PeriodicExportingMetricReader, and a BatchLogRecordProcessor, then bridge console output into the logger so Next.js internal SSR errors become log records."},{"@type":"HowToStep","name":"Mount browser telemetry","text":"Register a WebTracerProvider with the ZoneContextManager from a client component in the root layout, add the web auto-instrumentations for document load, fetch, XHR, and clicks, and wire error handlers and Web Vitals."},{"@type":"HowToStep","name":"Proxy browser OTLP through an API route","text":"Add a catch-all route at /api/otel/[...signal] that forwards the request body to the collector, and point the browser exporter at it so telemetry is posted same-origin with no collector CORS."},{"@type":"HowToStep","name":"Run and verify","text":"Start the stack, load a page, click through the demo routes, and confirm the server service and the browser service both report spans, with browser fetch spans parenting the API route spans they triggered."}]})}
  </script>
</head>
...
### Do I need `output: "standalone"` for OpenTelemetry to work in Docker?
