...
Register a custom Monolog processor that reads the current span context and
injects `trace_id` and `span_id` into every log record. With JSON log
formatting, Scout can then match logs to their traces.
