...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"How does LangChain OpenTelemetry tracing differ from LangSmith?","acceptedAnswer":{"@type":"Answer","text":"LangSmith gives deep LangChain-specific traces but sits apart from your HTTP, database, and infrastructure telemetry. OpenTelemetry produces one trace across every layer, so a slow /diagnose response links to the exact LLM call, tool invocation, vector query, and SQL INSERT that caused it. It is also vendor-neutral: the same spans flow to base14 Scout or any OTLP backend."}},{"@type":"Question","name":"Does a LangChain callback handler add latency?","acceptedAnswer":{"@type":"Answer","text":"No. Starting and ending a span takes microseconds; LLM calls take seconds. The handler only stores a run_id to span map and sets attributes. Spans export in a background BatchSpanProcessor thread, off the request path."}},{"@type":"Question","name":"Which LangChain versions does this guide support?","acceptedAnswer":{"@type":"Answer","text":"LangChain 1.3+ with langchain-core 1.4+, where the agent is built with create_agent and is LangGraph-backed. The callback API (BaseCallbackHandler and the on_chain / on_chat_model / on_tool / on_retriever events) has been stable across the 1.x line."}},{"@type":"Question","name":"How do I trace tool calls in a LangChain agent?","acceptedAnswer":{"@type":"Answer","text":"Implement on_tool_start and on_tool_end in a callback handler and emit an execute_tool span with gen_ai.tool.name and gen_ai.tool.type. The zero-code OpenLLMetry library does this for you; the custom handler in this guide shows the exact mapping."}},{"@type":"Question","name":"How do I track LLM cost with LangChain and OpenTelemetry?","acceptedAnswer":{"@type":"Answer","text":"Read usage_metadata in on_llm_end for input and output token counts, multiply by a per-model price table, and record a gen_ai.client.cost counter alongside the gen_ai.client.token.usage histogram. Attach gen_ai.request.model and gen_ai.provider.name so you can group cost by model in dashboards."}},{"@type":"Question","name":"Can I capture prompts and completions in LangChain traces?","acceptedAnswer":{"@type":"Answer","text":"Yes, but it is off by default. The custom handler captures PII-scrubbed content only when OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=true. The OpenLLMetry auto-instrumentation captures content by default and is controlled by its own TRACELOOP_TRACE_CONTENT variable, so set TRACELOOP_TRACE_CONTENT=false to disable it there."}},{"@type":"Question","name":"Should I use auto-instrumentation or a custom callback handler?","acceptedAnswer":{"@type":"Answer","text":"Start with OpenLLMetry auto-instrumentation for traces in minutes and no code changes. Move to a custom callback handler when you need clean gen_ai semantic-convention span names, correct model and finish-reason fields, custom retrieval attributes, or content capture that is off by default. Both work by injecting a LangChain callback handler."}},{"@type":"Question","name":"Why does that package come from Traceloop, not OpenTelemetry?","acceptedAnswer":{"@type":"Answer","text":"The PyPI name opentelemetry-instrumentation-langchain belongs to Traceloop's OpenLLMetry, not the official OpenTelemetry GenAI SIG. That package is mature and exports to any OTLP collector. The official SIG instrumentation is still distributed from the opentelemetry-python-contrib source tree and is the emerging gen_ai-native standard to track."}},{"@type":"Question","name":"How do MCP tools fit, and are they traced?","acceptedAnswer":{"@type":"Answer","text":"In-process LangChain tools are traced by the callback handler as execute_tool spans. When tools live behind a Model Context Protocol server, the OpenTelemetry MCP span conventions cover that hop, which is a separate integration from the LangChain callback handler."}}]})}
  </script>
</head>
