...
sidebar_position: 38
description: >
  Collect MariaDB metrics with the OpenTelemetry Collector. Monitor query
...
The OpenTelemetry Collector's MySQL receiver also supports MariaDB —
both use the MySQL wire protocol. It collects 21+ metrics from
MariaDB 10.5+, including query performance, connection counts, table
lock waits, replication lag, and statement event statistics. This guide
configures the receiver, sets up a monitoring user, and ships metrics
to base14 Scout.
...
- MariaDB must be accessible from the host running the Collector
...
- **Connections**: connection count, connection errors, thread states
...
mariadb -h <mariadb-host> -P <port> -u otel_monitor -p -e "SELECT version();"
```
...
**Cause**: Collector cannot reach MariaDB at the configured endpoint.
...
Yes. Add multiple receiver blocks with distinct names for each node:
...
`receivers: [mysql/node1, mysql/node2]`

**Does this work with MariaDB running in Kubernetes?**

Yes. Set `endpoint` to the MariaDB service DNS
(e.g., `mariadb.default.svc.cluster.local:3306`) and inject credentials
...
- **Monitor More Components**: Add monitoring for
  [MySQL](./mysql.md), [PostgreSQL](./postgres.md),
