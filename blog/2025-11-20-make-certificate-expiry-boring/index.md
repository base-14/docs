...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Why do certificate expiry outages still happen?","acceptedAnswer":{"@type":"Answer","text":"Certificate expiry behaves like a trapdoor: everything works perfectly until the moment it doesn't. Ownership is often ambiguous across teams, certificates accumulate in forgotten corners of the system, and expiry is rarely treated as a first-class operational metric."}},{"@type":"Question","name":"How should you alert on TLS certificate expiry?","acceptedAnswer":{"@type":"Answer","text":"Use staggered alert windows: 30 days as a non-paging warning, 14 days as urgent, and 7 days as critical and paging. Also alert on renewal failures - ACME challenge errors, CA issuance failures, permissions regressions - since a certificate expiring is usually a symptom of broken automation, not a missed calendar reminder."}},{"@type":"Question","name":"How do you rotate TLS certificates without downtime?","acceptedAnswer":{"@type":"Answer","text":"Ensure an overlap window where both old and new certificates are valid and deployed simultaneously. Use atomic attachment for load balancers, prefer graceful reloads over full restarts, and validate the entire certificate chain after rotation completes."}},{"@type":"Question","name":"How do you find all TLS certificates in your infrastructure?","acceptedAnswer":{"@type":"Answer","text":"Certificates are spread across public entry points like API gateways and load balancers, internal service-to-service mTLS, databases and message brokers, cloud-managed infrastructure, and often-forgotten legacy components like VPN servers, staging environments, and batch jobs. Build an inventory by scanning all endpoints, not just the ones you remember."}},{"@type":"Question","name":"How do you automate certificate renewal across environments?","acceptedAnswer":{"@type":"Answer","text":"Use ACME-based automation (Let's Encrypt or internal ACME servers) for automatic request, validation, and renewal. In Kubernetes, cert-manager handles the full lifecycle. Cloud providers auto-renew but need monitoring for silent failures like DNS misconfigurations."}}]})}
  </script>
</head>
