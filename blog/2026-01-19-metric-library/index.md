...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"How do you find what metrics OpenTelemetry Collector emits?","acceptedAnswer":{"@type":"Answer","text":"Check the metadata.yaml file in each receiver's directory in the opentelemetry-collector-contrib repository. A metric registry that indexes all receivers automatically can help - the OTel Collector Contrib repo has over 100 receivers, each emitting dozens of metrics that change with every release."}},{"@type":"Question","name":"How many metrics does OpenTelemetry Collector have?","acceptedAnswer":{"@type":"Answer","text":"The OpenTelemetry Collector Contrib repository contains over 100 receivers, each emitting dozens to hundreds of metrics. Combined with Semantic Conventions, SDK instrumentation libraries, and Prometheus exporters, the ecosystem contains well over 3,700 distinct metric definitions that change with every release."}},{"@type":"Question","name":"How do you check if your metrics follow OpenTelemetry Semantic Conventions?","acceptedAnswer":{"@type":"Answer","text":"Cross-reference your metric names against the OpenTelemetry Semantic Conventions specification, which defines 349 standard metrics. Look for exact name matches and correct unit and type attributes. Tools like metric registries can automate this by labeling each metric as exact match, prefix match, or non-compliant."}},{"@type":"Question","name":"What Prometheus exporters are available for databases?","acceptedAnswer":{"@type":"Answer","text":"The Prometheus ecosystem includes dedicated exporters for PostgreSQL (postgres_exporter), MySQL (mysql_exporter), MongoDB (mongodb_exporter), Redis (redis_exporter), and Kafka (kafka_exporter). Each exposes database-specific metrics via the standard /metrics endpoint that can be scraped by an OpenTelemetry Collector or Prometheus server."}},{"@type":"Question","name":"What is the difference between OpenTelemetry metrics and Prometheus metrics?","acceptedAnswer":{"@type":"Answer","text":"OpenTelemetry defines metrics through Semantic Conventions with standardized names, units, and attributes. Prometheus metrics are defined by individual exporters with no enforced naming standard. OpenTelemetry metrics can be exported in Prometheus format, but the naming conventions, attribute schemas, and type mappings differ between the two ecosystems."}}]})}
  </script>
</head>
