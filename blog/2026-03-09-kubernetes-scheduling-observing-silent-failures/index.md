...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Why do Kubernetes pods get stuck in Pending state?","acceptedAnswer":{"@type":"Answer","text":"A Pending pod means the scheduler cannot place it on any node or it is waiting on a dependency like volume binding. The five most common causes are resource requests exceeding capacity, overly restrictive placement constraints, unbound PersistentVolumeClaims, cluster autoscaler limits, and namespace ResourceQuota exhaustion."}},{"@type":"Question","name":"How do you debug a Pending pod in Kubernetes?","acceptedAnswer":{"@type":"Answer","text":"Start with kubectl describe pod to read the FailedScheduling event message. Then check PVC status with kubectl get pvc, inspect node capacity with kubectl describe nodes, and review cluster autoscaler logs. The FailedScheduling event typically provides the direct explanation."}},{"@type":"Question","name":"How can OpenTelemetry detect Kubernetes scheduling failures?","acceptedAnswer":{"@type":"Answer","text":"Deploy the OTel Collector with the k8s_cluster receiver for pod phase and node condition metrics, kubeletstats for per-node resource utilization, and k8sobjects to watch FailedScheduling events as structured log records. Alert on Pending pod duration exceeding 2-3 minutes and on rising FailedScheduling event rates."}},{"@type":"Question","name":"What OTel Collector deployment topology is recommended for Kubernetes?","acceptedAnswer":{"@type":"Answer","text":"Use a DaemonSet deployment on every node running the kubeletstats receiver for per-node resource metrics, plus a single-replica Deployment running the k8s_cluster and k8sobjects receivers for cluster-level state."}},{"@type":"Question","name":"What Kubernetes metrics should you monitor to prevent scheduling failures?","acceptedAnswer":{"@type":"Answer","text":"Monitor k8s.pod.phase filtered for Pending, k8s.node.condition for MemoryPressure and DiskPressure, k8s.node.allocatable_cpu and allocatable_memory for headroom, and k8s.resource_quota.used versus hard_limit. Alert when allocatable headroom drops below 10-15%."}}]})}
  </script>
</head>
