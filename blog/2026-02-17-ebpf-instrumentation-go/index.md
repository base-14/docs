...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Why is auto-instrumentation different for Go compared to Java or Python?","acceptedAnswer":{"@type":"Answer","text":"Go compiles directly to machine code with no virtual machine, interpreter, or class loader. JVM-style agent injection has nothing to attach to. Instead, Go uses eBPF-based runtime probes or compile-time tools that rewrite source code during the build."}},{"@type":"Question","name":"How does eBPF instrumentation work for Go applications?","acceptedAnswer":{"@type":"Answer","text":"eBPF attaches uprobes to function entry points and uretprobes to return points in a running Go binary. The agent reads the binary's symbol table, captures arguments and timestamps, and constructs OpenTelemetry spans exported via OTLP. No code changes, recompilation, or process restart required."}},{"@type":"Question","name":"What is the difference between opentelemetry-go-instrumentation and OBI?","acceptedAnswer":{"@type":"Answer","text":"opentelemetry-go-instrumentation uses uprobes on Go library functions and is Go-specific. OBI originated as Grafana Beyla and was donated to OpenTelemetry in May 2025. It works at the network protocol layer, making it language-agnostic - OBI supports any language using HTTP, gRPC, SQL, or Redis, while the OTel Go agent provides deeper Go-specific span detail."}},{"@type":"Question","name":"What are the security requirements for eBPF Go instrumentation on Kubernetes?","acceptedAnswer":{"@type":"Answer","text":"The sidecar must run as root with privileged: true to load eBPF programs, and shareProcessNamespace must be enabled. OBI offers a lighter alternative using fine-grained capabilities like CAP_BPF and CAP_PERFMON instead of full privileged mode."}},{"@type":"Question","name":"What is the biggest limitation of eBPF instrumentation for Go?","acceptedAnswer":{"@type":"Answer","text":"Context propagation. eBPF can inject traceparent headers into outgoing HTTP and gRPC calls, but within a service, Go code must pass context.Context through function calls. Without it, spans like database queries appear as disconnected top-level traces."}}]})}
  </script>
</head>
