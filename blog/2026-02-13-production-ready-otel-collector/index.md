...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Why does the OpenTelemetry Collector need hardening for production?","acceptedAnswer":{"@type":"Answer","text":"Default settings ship without memory limits, retry logic, queue sizing, or self-monitoring. The collector will accept data until it runs out of memory, drop data silently when queues fill up, and give no signal that anything went wrong."}},{"@type":"Question","name":"Why must the memory_limiter be the first processor in an OTel Collector pipeline?","acceptedAnswer":{"@type":"Answer","text":"If the memory_limiter comes after the batch processor, data has already been buffered before the limiter can act. Placing it first ensures backpressure is applied before any data accumulates, preventing out-of-memory crashes."}},{"@type":"Question","name":"How do you diagnose silent data loss in the OpenTelemetry Collector?","acceptedAnswer":{"@type":"Answer","text":"Check the internal metrics at localhost:8888/metrics. If otelcol_exporter_queue_size equals otelcol_exporter_queue_capacity, data is being dropped. Compare otelcol_receiver_accepted_spans against otelcol_exporter_sent_spans to find where data is lost in the pipeline."}},{"@type":"Question","name":"What causes broken traces in the OpenTelemetry Collector?","acceptedAnswer":{"@type":"Answer","text":"Broken traces are usually caused by context propagation failures: an intermediate service not propagating the traceparent header, multiple collectors exporting to different backends with inconsistent resource attributes, or a load balancer stripping trace context headers."}},{"@type":"Question","name":"What is the production checklist for the OpenTelemetry Collector?","acceptedAnswer":{"@type":"Answer","text":"Key items: memory_limiter as the first processor in every pipeline, batch processor with appropriate sizing, retry_on_failure enabled on all exporters, sending_queue enabled, health_check extension wired to container probes, internal metrics being scraped, and debug exporter disabled."}}]})}
  </script>
</head>
