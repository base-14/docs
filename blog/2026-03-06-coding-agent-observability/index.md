...
<head>
  <script type="application/ld+json">
    {JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Which coding agents support OpenTelemetry?","acceptedAnswer":{"@type":"Answer","text":"Claude Code, OpenAI Codex CLI, and Google Gemini CLI all support native OpenTelemetry export. Claude Code emits metrics and logs, while Codex CLI and Gemini CLI emit traces, metrics, and logs. Gemini CLI has the most comprehensive instrumentation, following the OpenTelemetry GenAI Semantic Conventions."}},{"@type":"Question","name":"How do you enable telemetry for Claude Code?","acceptedAnswer":{"@type":"Answer","text":"Set CLAUDE_CODE_ENABLE_TELEMETRY=1 and point OTEL_EXPORTER_OTLP_ENDPOINT to your collector. Claude Code emits metrics (token usage, cost, lines of code) and log events (API requests, tool results, errors) via OTLP. Route these to any OpenTelemetry-compatible backend to track per-engineer spend and model routing."}},{"@type":"Question","name":"How do you track AI coding assistant costs per developer?","acceptedAnswer":{"@type":"Answer","text":"Enable OpenTelemetry export in your coding agent and route metrics to a collector. Claude Code emits per-model cost and token usage metrics, letting you break down spend by engineer, project, and model. Without this telemetry, cost attribution is a guess based on monthly totals."}},{"@type":"Question","name":"How many tokens does Claude Code use per session?","acceptedAnswer":{"@type":"Answer","text":"Token usage varies significantly by task and engineer. In a seven-day study across five engineers, cache read tokens dominated at 289 million versus 4.23 million input tokens. Haiku handled over half of API requests while Opus handled 43%. One engineer accounted for nearly half of total token usage, showing how widely usage patterns differ."}},{"@type":"Question","name":"How do you set up OpenTelemetry for Codex CLI?","acceptedAnswer":{"@type":"Answer","text":"Configure the [otel] section in ~/.codex/config.toml with your collector endpoint. Codex CLI emits traces, metrics, and logs via OTLP, including token usage, tool calls, and session data. Unlike Claude Code which only emits metrics and logs, Codex CLI includes full distributed traces for each agent interaction."}}]})}
  </script>
</head>
